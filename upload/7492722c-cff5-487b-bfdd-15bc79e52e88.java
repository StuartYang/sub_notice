...
public class NoticeController {
    ...
    public R addSaveNotice(HttpSession session,Tnotice notice) {
    	...
    	notice.setId((long)1);
    	notice.setTnType(1);
    	notice.setTuId((long)tuser.getId());
    ...
    @ResponseBody
    public R editNotice(@PathVariable("id") Long id) {
        Tnotice tnotice = new Tnotice();
        tnotice = noticeService.selectByPrimaryKey(id);
        if (tnotice != null) {
    ...
    @PostMapping("/select")
    @ResponseBody
    public R selectNotice(@Param("id") Long id) {
        Tnotice tnotice = new Tnotice();
        tnotice = noticeService.selectByPrimaryKey(id);
...
//    public String selectNotice(@PathVariable("id") Long id ,Model model) {
